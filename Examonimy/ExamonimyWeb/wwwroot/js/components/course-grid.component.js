...
export class CourseGridComponent extends BaseComponent {
    ...
    #render() {     
        ...
        <label id="course-label-${course.id}" data-id="${course.id}" data-course-code="${course.courseCode}" class="course-label relative flex cursor-pointer rounded-lg bg-white p-4 focus:outline-none">         
            <span class="flex flex-1">
                <span class="flex flex-col">                  
                    <span class="course-name block text-sm font-medium text-yellow-600">${course.name}</span>
            ...
            <svg class="course-checkbox h-5 w-5 text-yellow-600 invisible" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
    ...
    #highlightCourse(courseLabelElement = new HTMLElement()) {
        ...
        courseLabels.forEach(courseLabel => {
            courseLabel.classList.remove("bg-yellow-200");   
            courseLabel.classList.add("bg-white");
        ...
        courseLabelElement.classList.remove("bg-white");
        courseLabelElement.classList.add("bg-yellow-200");     
