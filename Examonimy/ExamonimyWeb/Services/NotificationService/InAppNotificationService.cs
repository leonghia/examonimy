...
namespace ExamonimyWeb.Services.NotificationService
{
    ...
    <div class=""absolute flex items-center justify-center w-5 h-5 ms-6 -mt-5 bg-amber-600 border border-white rounded-full dark:border-gray-800"">                                       <svg xmlns=""http://www.w3.org/2000/svg"" viewBox=""0 0 24 24"" fill=""currentColor"" class=""w-2 h-2 text-white"">
        ...
        public async Task<string> GetMessageMarkupAsync(Notification notification)
        {
            ...
            {
                case NotificationTypeIds.AskForReviewForExamPaper:
                    var course = await _courseRepository.GetByIdAsync(notification.EntityId) ?? throw new ArgumentException(null, nameof(notification.EntityId));
                    var courseName = course.Name;
                    return new AskForReviewForExamPaperNotiMessage(actorFullName, courseName).ToVietnamese();
                default:
                    throw new SwitchExpressionException(notification.NotificationTypeId);
